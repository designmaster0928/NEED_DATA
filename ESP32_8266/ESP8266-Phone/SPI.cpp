...
bool ICACHE_RAM_ATTR SPIClass::pins(int8_t sck, int8_t miso, int8_t mosi, int8_t ss)
{
    if (sck == 6 &&
        miso == 7 &&
        mosi == 8 &&
...
void ICACHE_RAM_ATTR SPIClass::setHwCs(bool use) {
    switch (pinSet) {
    case SPI_PINS_HSPI:
        if (use) {
...
void ICACHE_RAM_ATTR SPIClass::beginTransaction(SPISettings settings) {
...
void ICACHE_RAM_ATTR SPIClass::endTransaction() {
}

void ICACHE_RAM_ATTR SPIClass::setDataMode(uint8_t dataMode) {
...
void ICACHE_RAM_ATTR SPIClass::setBitOrder(uint8_t bitOrder) {
 ...
 * @return
 */
static ICACHE_RAM_ATTR uint32_t ClkRegToFreq(spiClk_t * reg) {
    return (ESP8266_CLOCK / ((reg->regPre + 1) * (reg->regN + 1)));
}

void ICACHE_RAM_ATTR SPIClass::setFrequency(uint32_t freq) {
...
void ICACHE_RAM_ATTR SPIClass::setClockDivider(uint32_t clockDiv) {
...
inline void ICACHE_RAM_ATTR SPIClass::setDataBits(uint16_t bits)
{
...
uint8_t ICACHE_RAM_ATTR SPIClass::transfer(uint8_t data)
{
    while(SPI1CMD & SPIBUSY) {}
    // reset to 8Bit mode
    //setDataBits(8); // calling setdatabits here crashes...
    const uint32_t mask = ~((SPIMMOSI << SPILMOSI) | (SPIMMISO << SPILMISO)); // but this doesn't. Same code.
...
uint16_t ICACHE_RAM_ATTR SPIClass::transfer16(uint16_t data) {
    union {
            uint16_t val;
            struct {
                    uint8_t lsb;
...
void ICACHE_RAM_ATTR SPIClass::transfer(void *buf, uint16_t count) {
...
void ICACHE_RAM_ATTR SPIClass::write16(uint16_t data) {
    write16(data, !(SPI1C & (SPICWBO | SPICRBO)));
}

void ICACHE_RAM_ATTR SPIClass::write16(uint16_t data, bool msb) {
    while(SPI1CMD & SPIBUSY) {}
...
void ICACHE_RAM_ATTR SPIClass::write32(uint32_t data) {
    write32(data, !(SPI1C & (SPICWBO | SPICRBO)));
}

void ICACHE_RAM_ATTR SPIClass::write32(uint32_t data, bool msb) {
    while(SPI1CMD & SPIBUSY) {}
 ...
 * @param size uint32_t
 */
void ICACHE_RAM_ATTR SPIClass::writeBytes(const uint8_t * data, uint32_t size) {
    while(size) {
        if(size > 64) {
            writeBytes_(data, 64);
            size -= 64;
...
void ICACHE_RAM_ATTR SPIClass::writeBytes_(const uint8_t * data, uint8_t size) {
    while(SPI1CMD & SPIBUSY) {}
 ...
 */
void ICACHE_RAM_ATTR SPIClass::writePattern(const uint8_t * data, uint8_t size, uint32_t repeat) {
 ...
 * @param size uint32_t
 */
void ICACHE_RAM_ATTR SPIClass::transferBytes(const uint8_t * out, uint8_t * in, uint32_t size) {
    while(size) {
        if(size > 64) {
...
void ICACHE_RAM_ATTR SPIClass::transferBytesAligned_(const uint8_t * out, uint8_t * in, uint8_t size) {
    if (!size)
        return;
...
void ICACHE_RAM_ATTR SPIClass::transferBytes_(const uint8_t * out, uint8_t * in, uint8_t size) {
